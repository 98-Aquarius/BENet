...
class SR(nn.Module):
    def __init__(self, N, d_model=512, seq_len = 49):
        ...
        # )
        #self.gated = nn.GLU()
        #self.lin = nn.Linear(N * d_model, 2 * d_model)
    ...
    def forward(self, x, layers, attention_mask = None, attention_weights = None):
        ...
        #gate1 = self.gated(outs[0])
        #gate2 = self.gated(outs[1])
        #out = torch.cat((gate1, gate2, outs[2]),-1)
...
class MultiLevelEncoder(nn.Module):
    ...
    def forward(self, input, attention_weights=None):
        ...
        # #Gate
        # gate = torch.sigmoid(input)
        # input = torch.mul(input, gate)
